...
class Client extends EventEmitter {
    ...
    async inject() {
        ...
        const needAuthentication = await this.pupPage.evaluate(async () => {
            ...
            return { need: state == 'UNPAIRED' || state == 'UNPAIRED_IDLE', state, hasSynced: window.require('WAWebSocketModel').Socket.hasSynced };
        });
        ...
        await exposeFunctionIfAbsent(
            ...
            async () => {
                ...
                if (!injected) {
                    ...
                    // Inject diagnostic common (sets up window.__diag)
                    ...
                    console.log('[wwjs-diag] onAppStateHasSyncedEvent WWebJS ready', JSON.stringify({ durationMs: Date.now() - start }));
        ...
        await this.pupPage.evaluate(() => {
            ...
            // Store Socket reference to detect replacement
            window._wwjsDiagAppState = window.require('WAWebSocketModel').Socket;
            ...
            window
                ...
                .Socket.on('change:hasSynced', () => {
                    console.error('[wwjs-diag] change:hasSynced FIRED ' + JSON.stringify({
                        hasSynced: window.require('WAWebSocketModel').Socket.hasSynced,
                        state: window.require('WAWebSocketModel').Socket.state,
                        sameAppState: window.require('WAWebSocketModel').Socket === window._wwjsDiagAppState,
                        ts: Date.now()
                    }));
        ...
        // Fix race condition: check hasSynced after all listeners are registered
        try {
            const syncCheck = await this.pupPage.evaluate(() => {
                const Socket = window.require('WAWebSocketModel').Socket;
                return {
                    socketAvailable: !!Socket,
                    hasSynced: Socket && Socket.hasSynced,
            ...
            if (syncCheck.socketAvailable && syncCheck.hasSynced === true) {
    ...
    async initialize() {
        ...
        // [diag:promise-collected] Track concurrent evaluate calls
    ...
    async attachEventListeners() {
        ...
        await this.pupPage.evaluate(() => {
            ...
            // Track message IDs handled by the normal 'add' path
            ...
            // [SILENT_LOSS_FIX] Fallback: catch messages that bypassed 'add' event
        ...
        // [L7] Verify Msg listener registration succeeded
        ...
        this.emit('diag', 'info', 'Msg listener count', JSON.stringify(listenerCount));
    }
